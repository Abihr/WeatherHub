const express = require("express");
const cors = require("cors");
const dotenv = require("dotenv");
const path = require("path");
const { getAgricultureData } = require("./agricultureService");

dotenv.config({
    path: path.resolve(__dirname, ".env"),
});

const app = express();

app.use(cors({ origin: "*" }));
app.use(express.json());

console.log(
    "🔑 WEATHER_API_KEY:",
    process.env.WEATHER_API_KEY ? "Loaded ✅" : "Missing ❌"
);

// ======================================================
// OPENWEATHER CURRENT WEATHER
// ======================================================

async function getWeatherByCoordinates(latitude, longitude) {
    try {
        const apiKey = process.env.WEATHER_API_KEY;

        if (!apiKey) {
            throw new Error("WEATHER_API_KEY is not configured");
        }

        const url =
            `https://api.openweathermap.org/data/2.5/weather` +
            `?lat=${encodeURIComponent(latitude)}` +
            `&lon=${encodeURIComponent(longitude)}` +
            `&appid=${encodeURIComponent(apiKey)}` +
            `&units=metric`;

        console.log("🌤️ Calling OpenWeather:", latitude, longitude);

        const response = await fetch(url);
        const text = await response.text();

        let data;

        try {
            data = text ? JSON.parse(text) : {};
        } catch (error) {
            console.error("❌ Invalid JSON from OpenWeather:");
            console.error(text);
            throw new Error("Invalid response from OpenWeather");
        }

        if (!response.ok) {
            console.error(
                "❌ OpenWeather error:",
                response.status,
                data
            );

            throw new Error(
                data?.message ||
                `OpenWeather returned status ${response.status}`
            );
        }

        const weatherIcon = data.weather?.[0]?.icon || "01d";

        // OpenWeather:
        // 01d = day
        // 01n = night
        const is_day = weatherIcon.endsWith("n") ? 0 : 1;

        return {
            location: data.name || "Unknown",
            country: data.sys?.country || "",

            latitude,
            longitude,

            temperature: data.main?.temp ?? null,
            feelsLike: data.main?.feels_like ?? null,
            humidity: data.main?.humidity ?? null,
            pressure: data.main?.pressure ?? null,

            windSpeed: data.wind?.speed ?? null,
            windDirection: data.wind?.deg ?? null,

            condition:
                data.weather?.[0]?.description || "Unknown",

            weatherMain:
                data.weather?.[0]?.main || "Unknown",

            // IMPORTANT
            weatherIcon,

            is_day,

            rainfall:
                data.rain?.["1h"] ??
                data.rain?.["3h"] ??
                0,

            visibility: data.visibility ?? null,
        };
    } catch (error) {
        console.error(
            "❌ getWeatherByCoordinates error:",
            error
        );

        throw error;
    }
}

// ======================================================
// OPEN-METEO FORECAST
// ======================================================

async function getForecastByCoordinates(latitude, longitude) {
    try {
        const url =
            `https://api.open-meteo.com/v1/forecast` +
            `?latitude=${encodeURIComponent(latitude)}` +
            `&longitude=${encodeURIComponent(longitude)}` +
            `&timezone=Asia%2FKolkata` +
            `&forecast_days=7` +

            // Current
            `&current=` +
            `temperature_2m,` +
            `relative_humidity_2m,` +
            `apparent_temperature,` +
            `weather_code,` +
            `is_day` +

            // Hourly
            `&hourly=` +
            `temperature_2m,` +
            `relative_humidity_2m,` +
            `rain,` +
            `precipitation,` +
            `precipitation_probability,` +
            `weather_code,` +
            `is_day,` +
            `wind_gusts_10m` +

            // Daily
            `&daily=` +
            `temperature_2m_max,` +
            `temperature_2m_min,` +
            `rain_sum,` +
            `precipitation_probability_max,` +
            `weather_code`;

        console.log("🌦️ Calling Open-Meteo:", url);

        const response = await fetch(url);

        if (!response.ok) {
            throw new Error(
                `Open-Meteo returned status ${response.status}`
            );
        }

        const data = await response.json();

        const current = data.current || {};
        const hourly = data.hourly || {};
        const daily = data.daily || {};

        // ==================================================
        // FIND CURRENT LOCAL HOUR
        // ==================================================

        const hourlyTimes = hourly.time || [];

        const now = new Date();

        const localParts = new Intl.DateTimeFormat("en-CA", {
            timeZone: "Asia/Kolkata",
            year: "numeric",
            month: "2-digit",
            day: "2-digit",
            hour: "2-digit",
            hourCycle: "h23",
        }).formatToParts(now);

        const local = {};

        localParts.forEach((part) => {
            if (part.type !== "literal") {
                local[part.type] = part.value;
            }
        });

        const currentLocalTime =
            `${local.year}-${local.month}-${local.day}T${local.hour}:00`;

        console.log(
            "🕐 Current local hour:",
            currentLocalTime
        );

        let startIndex = hourlyTimes.findIndex(
            (time) => String(time).slice(0, 13) === currentLocalTime.slice(0, 13)
        );

        if (startIndex === -1) {
            console.warn(
                "⚠️ Current hour not found. Using first available hour."
            );
            startIndex = 0;
        }

        // ==================================================
        // HOURLY FORECAST
        // ==================================================

        const hourlyForecast = [];

        for (
            let i = startIndex;
            i < Math.min(startIndex + 24, hourlyTimes.length);
            i++
        ) {
            const isDay =
                hourly.is_day?.[i] !== undefined &&
                hourly.is_day?.[i] !== null
                    ? Number(hourly.is_day[i])
                    : null;

            const hourData = {
                time: hourly.time?.[i] ?? null,

                temperature:
                    hourly.temperature_2m?.[i] ?? null,

                humidity:
                    hourly.relative_humidity_2m?.[i] ?? null,

                rain:
                    hourly.rain?.[i] ?? 0,

                precipitation:
                    hourly.precipitation?.[i] ?? 0,

                rainProbability:
                    hourly.precipitation_probability?.[i] ?? 0,

                weatherCode:
                    hourly.weather_code?.[i] ?? null,

                // ⭐ IMPORTANT
                is_day: isDay,

                windGust:
                    hourly.wind_gusts_10m?.[i] ?? null,
            };

            hourlyForecast.push(hourData);

            console.log(
                `⏰ ${hourData.time} | is_day=${hourData.is_day} | code=${hourData.weatherCode}`
            );
        }

        // ==================================================
        // DAILY FORECAST
        // ==================================================

        const dailyForecast = [];

        const dailyTimes = daily.time || [];

        for (let i = 0; i < dailyTimes.length; i++) {
            dailyForecast.push({
                date:
                    daily.time?.[i] ?? null,

                minTemp:
                    daily.temperature_2m_min?.[i] ?? null,

                maxTemp:
                    daily.temperature_2m_max?.[i] ?? null,

                rain:
                    daily.rain_sum?.[i] ?? 0,

                rainProbability:
                    daily.precipitation_probability_max?.[i] ?? 0,

                weatherCode:
                    daily.weather_code?.[i] ?? null,

                // Daily card represents daytime
                is_day: 1,
            });
        }

        // ==================================================
        // CURRENT
        // ==================================================

        const currentIsDay =
            current.is_day !== undefined &&
            current.is_day !== null
                ? Number(current.is_day)
                : null;

        console.log(
            "🌞 Open-Meteo current is_day:",
            currentIsDay
        );

        return {
            current: {
                temperature:
                    current.temperature_2m ?? null,

                feelsLike:
                    current.apparent_temperature ?? null,

                humidity:
                    current.relative_humidity_2m ?? null,

                weatherCode:
                    current.weather_code ?? null,

                is_day:
                    currentIsDay,
            },

            hourly: hourlyForecast,

            daily: dailyForecast,

            timezone:
                data.timezone || "Asia/Kolkata",
        };
    } catch (error) {
        console.error(
            "❌ getForecastByCoordinates error:",
            error
        );

        throw error;
    }
}

// ======================================================
// WEATHER API
// ======================================================

app.get("/api/weather", async (req, res) => {
    try {
        const { lat, lon } = req.query;

        console.log("🌍 Weather request:", {
            lat,
            lon,
        });

        if (lat === undefined || lon === undefined) {
            return res.status(400).json({
                error: "Latitude and longitude are required",
            });
        }

        const latitude = Number(lat);
        const longitude = Number(lon);

        if (
            !Number.isFinite(latitude) ||
            !Number.isFinite(longitude)
        ) {
            return res.status(400).json({
                error: "Invalid latitude or longitude",
            });
        }

        if (latitude < -90 || latitude > 90) {
            return res.status(400).json({
                error:
                    "Latitude must be between -90 and 90",
            });
        }

        if (longitude < -180 || longitude > 180) {
            return res.status(400).json({
                error:
                    "Longitude must be between -180 and 180",
            });
        }

        // Get both APIs
        const [weather, forecast] = await Promise.all([
            getWeatherByCoordinates(
                latitude,
                longitude
            ),
            getForecastByCoordinates(
                latitude,
                longitude
            ),
        ]);

        // ==================================================
        // FINAL RESPONSE
        // ==================================================

        const result = {
            // Location
            location: weather.location,
            country: weather.country,

            latitude,
            longitude,

            timezone:
                forecast.timezone ||
                "Asia/Kolkata",

            // ==================================================
            // CURRENT WEATHER
            // ==================================================

            temperature:
                forecast.current.temperature ??
                weather.temperature,

            feelsLike:
                forecast.current.feelsLike ??
                weather.feelsLike,

            humidity:
                forecast.current.humidity ??
                weather.humidity,

            condition: weather.condition,

            weatherMain: weather.weatherMain,

            weatherIcon: weather.weatherIcon,

            // ⭐ IMPORTANT
            is_day:
                forecast.current.is_day ??
                weather.is_day,

            windSpeed: weather.windSpeed,

            windDirection:
                weather.windDirection,

            pressure: weather.pressure,

            rainfall: weather.rainfall,

            visibility: weather.visibility,

            // ==================================================
            // FORECAST
            // ==================================================

            current: {
                temperature:
                    forecast.current.temperature ??
                    weather.temperature,

                feelsLike:
                    forecast.current.feelsLike ??
                    weather.feelsLike,

                humidity:
                    forecast.current.humidity ??
                    weather.humidity,

                weatherCode:
                    forecast.current.weatherCode,

                is_day:
                    forecast.current.is_day,
            },

            hourly: forecast.hourly,

            daily: forecast.daily,
        };

        console.log(
            "✅ Weather fetched:",
            result.location,
            "|",
            result.temperature,
            "°C"
        );

        console.log(
            "🌞 Current is_day:",
            result.is_day
        );

        console.log(
            "🕐 Hourly is_day:",
            result.hourly.map(
                (hour) =>
                    `${hour.time}=${hour.is_day}`
            )
        );

        return res.json(result);

    } catch (error) {
        console.error(
            "❌ /api/weather error:",
            error
        );

        return res.status(500).json({
            error:
                error.message ||
                "Failed to fetch weather",
        });
    }
});

// ======================================================
// AGRICULTURE API
// ======================================================

app.get("/api/agriculture", async (req, res) => {
    try {
        const crops = String(req.query.crops || "")
            .split(",")
            .map((crop) => crop.trim())
            .filter(Boolean);

        const data = await getAgricultureData({
            city: req.query.city || "Pune",

            crops,

            farmName:
                req.query.farmName ||
                "Green Valley Farm",

            area:
                req.query.area ||
                "12 Acres",

            soilType:
                req.query.soilType ||
                "Black Soil",
        });

        return res.json(data);

    } catch (error) {
        console.error(
            "/api/agriculture error:",
            error
        );

        return res.status(500).json({
            error:
                error.message ||
                "Failed to fetch agriculture data",
        });
    }
});

// ======================================================
// RAILWAY WEATHER API
// ======================================================

app.get("/api/railway_weather", async (req, res) => {
    try {
        const latitude = Number(
            req.query.lat || 22.5726
        );

        const longitude = Number(
            req.query.lon || 88.3639
        );

        if (
            !Number.isFinite(latitude) ||
            !Number.isFinite(longitude)
        ) {
            return res.status(400).json({
                error: "Invalid latitude or longitude",
            });
        }

        const url =
            `https://api.open-meteo.com/v1/forecast` +
            `?latitude=${encodeURIComponent(latitude)}` +
            `&longitude=${encodeURIComponent(longitude)}` +
            `&current=` +
            `temperature_2m,` +
            `relative_humidity_2m,` +
            `apparent_temperature,` +
            `precipitation,` +
            `rain,` +
            `weather_code,` +
            `wind_speed_10m` +
            `&hourly=` +
            `temperature_2m,` +
            `precipitation_probability,` +
            `precipitation,` +
            `rain,` +
            `weather_code,` +
            `wind_speed_10m` +
            `&timezone=Asia%2FKolkata` +
            `&forecast_days=1`;

        console.log(
            "🚆 Railway weather request:",
            latitude,
            longitude
        );

        const response = await fetch(url);

        if (!response.ok) {
            throw new Error(
                `Open-Meteo returned status ${response.status}`
            );
        }

        const data = await response.json();

        return res.json(data);

    } catch (error) {
        console.error(
            "❌ /api/railway_weather error:",
            error
        );

        return res.status(500).json({
            error:
                error.message ||
                "Failed to fetch railway weather data",
        });
    }
});

// ======================================================
// TEST WEATHER API
// ======================================================

app.get("/api/test-weather", (req, res) => {
    res.json({
        message:
            "Weather server is working!",

        apiKey:
            process.env.WEATHER_API_KEY
                ? "Loaded"
                : "Missing",
    });
});

// ======================================================
// ROOT
// ======================================================

app.get("/", (req, res) => {
    res.json({
        message:
            "Weather API server is running",

        port:
            process.env.PORT ||
            5000,

        environment:
            process.env.NODE_ENV ||
            "development",
    });
});

// ======================================================
// SERVER
// ======================================================

const PORT =
    process.env.PORT || 5000;

app.listen(
    PORT,
    "0.0.0.0",
    () => {
        console.log(
            `🌤️ Weather server running on port ${PORT}`
        );
    }
);